...
int lv1_insert_htab_entry_355deh(uint64_t htab_id, uint64_t hpte_group, uint64_t hpte_v, uint64_t hpte_r, uint64_t bolted_flag,
                          uint64_t flags, uint64_t * hpte_index, uint64_t * hpte_evicted_v, uint64_t * hpte_evicted_r)
{
    ...
    __asm__ __volatile__("mr %%r3, %4;" "mr %%r4, %5;" "mr %%r5, %6;" "mr %%r6, %7;" "mr %%r7, %8;" "mr %%r8, %9;"
                         SYSCALL(HVSC_SYSCALL_355DEH) "mr %0, %%r3;" "mr %1, %%r4;" "mr %2, %%r5;" "mr %3, %%r6;":"=r"(ret),
...
int lv1_allocate_memory_355deh(uint64_t size, uint64_t page_size_exp, uint64_t flags, uint64_t * addr, uint64_t * muid)
{
    ...
    __asm__ __volatile__("mr %%r3, %3;" "mr %%r4, %4;" "li %%r5, 0;" "mr %%r6, %5;" SYSCALL(HVSC_SYSCALL_355DEH) "mr %0, %%r3;"
...
int lv1_undocumented_function_114_355deh(uint64_t start, uint64_t page_size, uint64_t size, uint64_t * lpar_addr)
{
    ...
    __asm__ __volatile__("mr %%r3, %2;" "mr %%r4, %3;" "mr %%r5, %4;" SYSCALL(HVSC_SYSCALL_355DEH) "mr %0, %%r3;"
...
void lv1_undocumented_function_115_355deh(uint64_t lpar_addr)
{
    ...
    __asm__ __volatile__("mr %%r3, %0;" SYSCALL(HVSC_SYSCALL_355DEH)
                         :      // no return registers
                         :"r"(lpar_addr)
                         :"r0", "r3", "r4", "r5", "r6", "r7", "r8", "r9", "r10", "r11", "r12", "lr", "ctr", "xer",
                         "cr0", "cr1", "cr5", "cr6", "cr7", "memory");
...
uint64_t lv2_alloc_355deh(uint64_t size, uint64_t pool)
{
    ...
    uint64_t original_syscall_code_1 = peekq(HVSC_SYSCALL_ADDR_355DEH);
    ...
    poke813(HVSC_SYSCALL_ADDR_355DEH, 0x7C0802A67C140378ULL);
    poke813(HVSC_SYSCALL_ADDR_355DEH + 8, 0x4BECB6317E80A378ULL);
    poke813(HVSC_SYSCALL_ADDR_355DEH + 16, 0x7C0803A64E800020ULL);

    uint64_t ret = 0;
    __asm__ __volatile__("mr %%r3, %1;" "mr %%r4, %2;" SYSCALL(HVSC_SYSCALL_355DEH) "mr %0, %%r3;":"=r"(ret)
                         :"r"(size), "r"(pool)
                         :"r0", "r3", "r4", "r5", "r6", "r7", "r8", "r9", "r10", "r11", "r12", "lr", "ctr", "xer",
                         "cr0", "cr1", "cr5", "cr6", "cr7", "memory");

    // restore original syscall code
    poke813(HVSC_SYSCALL_ADDR_355DEH, original_syscall_code_1);
